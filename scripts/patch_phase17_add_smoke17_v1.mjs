...
/**
 * Phase 17B
 * - Adds scripts/mcp_smoke_phase17.mjs
 * - Adds npm script mcp:smoke17
 *
 * Smoke17 asserts:
 *  - /tools.tools === /capabilities.tools (exact set match)
 *  - /tool policy: 400 missing ctx, 403 forbidden namespace, 404 unknown tool in allowed namespace
 *
...
const smokePath = path.join(ROOT, "scripts", "mcp_smoke_phase17.mjs");
mustExist(pkgPath);
...
if (pkg.scripts["mcp:smoke17"] !== "node scripts/mcp_smoke_phase17.mjs") {
  pkg.scripts["mcp:smoke17"] = "node scripts/mcp_smoke_phase17.mjs";
  fs.writeFileSync(pkgPath, JSON.stringify(pkg, null, 2) + "\n");
  console.log('Patched:', pkgPath, '(added "mcp:smoke17")');
  changed = true;
} else {
