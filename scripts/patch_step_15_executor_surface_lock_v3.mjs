...
// ------------------------------------------------------------
// 1) Write read-only executor surface lock audit
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// 3) Optional: neutralize the broken v2 patch script so nobody runs it by accident
// ------------------------------------------------------------
const broken = "scripts/patch_step_15_fix_executor_surface_patch_v2.mjs";
if (exists(broken)) {
  const safeStub = `#!/usr/bin/env node
/**
...
`;
  writeIfChanged(broken, safeStub);
  try { fs.chmodSync(broken, 0o755); } catch {}
  console.log("OK: stubbed broken patch_step_15_fix_executor_surface_patch_v2.mjs");
}
