...
if (src.includes("const ROOT = process.cwd();") &&
  ...
  console.log("OK: smoke_cross_domain_scope_spoofing already fixed (no-op).");
  process.exit(0);
}

// Match ANY heredoc ending with NODE
const rx = /node[\s\S]*?<<['"]?NODE['"]?[\s\S]*?\nNODE\b[\s\S]*?;/m;

if (!rx.test(src)) {
  console.error("ERROR: Could not locate NODE heredoc block in smoke_cross_domain_scope_spoofing.sh");
  process.exit(1);
}
...
console.log("Patched: smoke_cross_domain_scope_spoofing heredoc replaced with canonical ESM-safe block.");
