...
for (const d of domains) {
  ...
  for (const k of ["domain_id","executor_id","supported_task_types","required_scopes","validate_inputs","execute"]) {
    if (!(k in exec)) bad.push({ domain: d, error: "MISSING_KEY", key: k });
  }

  // Function shape constraints
...
if (!t.includes("./scripts/audit_executor_surface_lock_v1.sh")) {
  ...
  // Prefer to run after executor spec + action scopes + mount contract
