...
const OUT = path.join(ROOT, "docs", "AUTHORIZATION_INVARIANTS.md");

const content = `# CHC-Ops Authorization Invariants (Canonical)
...
// Idempotent write: write only if different
const prev = fs.existsSync(OUT) ? fs.readFileSync(OUT, "utf8") : null;
if (prev === content) {
  console.log("OK: docs/AUTHORIZATION_INVARIANTS.md already up to date (no-op).");
  process.exit(0);
}

fs.writeFileSync(OUT, content);
console.log("Wrote: docs/AUTHORIZATION_INVARIANTS.md");
