...
// Anchor on PASS assertion (must exist)
const PASS_ASSERT_RE = /assert\.(strictEqual|equal)\(\s*res\.status\s*,\s*200\s*\)/;

if (!PASS_ASSERT_RE.test(src)) {
  throw new Error("Unsafe: PASS assertion (res.status === 200) not found");
}
...
if (src.includes("contractVersion")) {
  ...
  // Inject ctx just before PASS assertion
  src = src.replace(
    PASS_ASSERT_RE,
